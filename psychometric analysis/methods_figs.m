...
all_data_converted = convert_story_types(all_data, "story_type", "story_categories.xlsx");

% divide by story

all_story_types = unique(all_data_converted.story_type);
for i = 1:length(all_story_types)
    story = all_story_types(i);
    all_task_data = all_data_converted(all_data_converted.story_type == story, :);
...
for j = 1:length(types)
    ...
    want_bdry = 1;
    want_scale = 1;
    type_data = rmoutliers(all_data_converted.(type));
    scale_max = max(type_data);
    scale_min = min(type_data);
    want_save = 1;
    for_ml = 0;
    path_to_save = "C:\Users\lrako\OneDrive\Documents\human_dm\final_run_2026\hr_figs_nonsense_adj\" ;
    mkdir(path_to_save)
...
for j = 1:length(types)
    type = types(j);
    save_to = "C:\Users\lrako\OneDrive\Documents\human_dm\final_run_2026\hr_figs_nonsense_adj\";
    mkdir(save_to)
    wanted_tasks = ["approach_avoid","social","probability","moral","supersense"];
