function normalized_human_to_rat_comparison_single_plot_2d(rat_table,human_cluster_table,save_to,b_dist_plots_dir,colors,normalize_or_dont)
...
if normalize_or_dont
    human_data = [human_cluster_table.clusterY,human_cluster_table.clusterZ];
    rescaled_human_data = normalize(human_data,"range",[0,1]);
    human_cluster_table.clusterY = rescaled_human_data(:,1);
    human_cluster_table.clusterZ = rescaled_human_data(:,2);

    rat_data = [rat_table.clusterY,rat_table.clusterZ];
    rescaled_rat_data = normalize(rat_data,"range",[0,1]);
    rat_table.clusterY = rescaled_rat_data(:,1);
    rat_table.clusterZ = rescaled_rat_data(:,2);
end
...
for i=1:length(rat_clusters)
    ...
    array_of_current_cluster_data_for_rat = [rat_current_cluster_info.clusterY,rat_current_cluster_info.clusterZ];
    % subplot(1,2,1);
    h = scatter(array_of_current_cluster_data_for_rat(:,1),array_of_current_cluster_data_for_rat(:,2),'o','MarkerEdgeColor',current_rat_color,'MarkerFaceColor',current_rat_color);
...
for j=1:length(human_clusters)
    ...
    array_of_current_cluster_data_for_human = [human_current_cluster_info.clusterY,human_current_cluster_info.clusterZ];
    h = scatter(array_of_current_cluster_data_for_human(:,1),array_of_current_cluster_data_for_human(:,2),'o','MarkerEdgeColor',current_human_color,'MarkerFaceColor',current_human_color);
    hs = [hs,h];
...
title("Rat vs Human 2D Cluster Comparison, " + to_add_to_title)
