    ...
    function[] = pig_helper(results, c, dirName, sig_type)
        if ~isempty(results)
            ...
            if length(y) >= 4 && all(~isnan(y))
                ...
                counter = 1;
                cannon_eq = "100/(1+exp(-a*(b*x - c*" + string(c) + ")))";
                [fitobject1, gof1] = fitlm(x.', y.', cannon_eq);
                ...
                counter = 1;
                new_eq = "100/(1+exp(-[a + d *" + story_pref + " ] * (b*x - c* " + string(c) + ")))";
                ...
                counter = 1;
                new_eq = "100/(1+exp(-[a + d *" + pupil_diam + " ] * (b*x - c* " + string(c) + ")))";
                [fitobject3, gof3] = fit(x.', y.', new_eq);
                ...
                save(strcat(dirName,'relevance_pig/',string(subid),"_",string(story_num),"_cost_",string(c),'.mat'),'fitobject2') 
                save(strcat(dirName,'pupil_pig/',string(subid),"_",string(story_num),"_cost_",string(c),'.mat'),'fitobject3') 

                save(strcat(dirName,'cannon_r/',string(subid),"_",string(story_num),"_cost_",string(c),'.mat'),'gof1') 
                save(strcat(dirName,'relevance_pig_r/',string(subid),"_",string(story_num),"_cost_",string(c),'.mat'),'gof2') 
                save(strcat(dirName,'pupil_pig_r/',string(subid),"_",string(story_num),"_cost_",string(c),'.mat'),'gof3') 
