function summary_table = create_indiv_summary_table(all_psych_data,total_table,tasks)

num_clusters = max(all_psych_data.idx);
summary_table = [];
ids = unique(all_psych_data.subjectidnumber);
for i = 1:height(ids)
    id = ids(i);
        ...
        for t2 = 1:length(tasks)
            ...
            for c1 = 1:num_clusters
                ...
                for c2 = 1:num_clusters
                    c_table = total_table(total_table.subjectnumberid == id & ...
                        total_table.task1 == task1 & total_table.task2 == task2 & ...
                        total_table.c1 == c1 & total_table.c2 == c2, :);
                    if ~isempty(c_table)
                        sample_vals = c_table.cond_prob;
