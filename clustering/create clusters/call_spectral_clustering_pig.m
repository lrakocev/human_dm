function [table_of_data, all_rs] = call_spectral_clustering_pig(table_of_dir,directory_where_cluster_table_should_be_saved,epsilon,given_number_of_clusters,colors,method,is_big,file_name,isolate_task)
...
for i=1:height(table_of_dir)
    [current_table,rs] = getTablePIG(table_of_dir{i,2},is_big);
    all_rs = [all_rs; rs];
...
for j=1:length(unique_indexes)
    ...
    three_d_cluster_table = getClusterTable3dWithExperiment(xVsYVsZ,raw_xVsYVsZ,labels,index,unique_indexes(j),table_of_data.F);
