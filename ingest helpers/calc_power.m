function [r_nout, c_nout] = calc_power(clean_approach_data)
...
for i = 1:length(clean_approach_data)
    ...
    r1 = mean((approach_data(approach_data.rew == 1,:).approach_rate),'omitnan');
    r4 = mean((approach_data(approach_data.rew == 4,:).approach_rate),'omitnan');
    
    c1 = mean((approach_data(approach_data.cost == 1,:).approach_rate),'omitnan');
    c4 = mean((approach_data(approach_data.cost == 4,:).approach_rate),'omitnan');
