function [adj_results, r_ratings, c_ratings, not_processed, next_save_for_later] = get_new_task(query, conn, table_name, save_for_later)

results = fetch(conn,query);
results = [results; save_for_later];

results = results(results.reward_level ~= "0", :);
...
for i = 1:length(unique_ids)
    ...
    for s = 1:length(unique_stories)
        ...
        if height(story_table) < 15
            next_save_for_later = [next_save_for_later; story_table];
            continue
        end
