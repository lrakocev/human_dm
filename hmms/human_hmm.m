...
filter_t_matrices = midway_hmm_rows(midway_hmm_rows.t_2 > 0.01 & midway_hmm_rows.t_4 > 0.01, :);
filter_t_matrices = sortrows(filter_t_matrices,"bic","ascend");

figure
scatter(filter_t_matrices.mpcs_1, filter_t_matrices.bic)
xlabel('mpcs')
ylabel('bics')
title('3 feature mpcs vs bics')

figure
histogram(filter_t_matrices.bic)
title('3 feature bics')

feature_combos = table(filter_t_matrices.features_1, filter_t_matrices.features_2, filter_t_matrices.features_3);
working_combos = unique(feature_combos,'rows');

%% check sequence

t_mat = [row.t_1 row.t_2; row.t_3 row.t_4];
e_mat = [row.e_1 row.e_2; row.e_3 row.e_4];
