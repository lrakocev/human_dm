...
% need to fix cause the old appr avoid is annoying
table_of_data = table_of_data(table_of_data.E ~= "old_approach_avoid", :);

%table_of_data.split = rowfun(@help_split, table_of_data,"InputVariables","D", ...
%                    "OutputVariableNames","split");
...
function split_row = help_split(row)

row = string(row);
if contains(row, "story")
    new_row = erase(row, "story_");
    split_row = split(new_row, ['_', '.']);
else
    split_row = split(row, ['_', '.']);
end
end
