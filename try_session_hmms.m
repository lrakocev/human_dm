...
for i = 1:height(all_combos)
    ...
    for k = 1:length(session_data)
        session_table = session_data{k};
        id = string(session_table.subjectidnumber(1));
        for j = 1:num_tries
            [states, bic, mpcs, t, e] = run_complete_hmm_process(session_table, id, curr_granularity, curr_features, curr_num_states);
           % hmm_row.state = states;
            ...
            if row_count < excel_limit
                writetable(hmm_row, "hmm_session_data_" + string(doc_num) + ".csv", 'WriteMode', 'append');
            else
                row_count = 0;
                doc_num = doc_num + 1;
                writetable(hmm_row, 'hmm_session_data_' + string(doc_num) + '.csv', 'WriteMode', 'append');
...
save("hmm_session_workspace.mat")
